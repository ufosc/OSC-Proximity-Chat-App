...
export const sendMessage = (
  ...
  if (
    message.location.lat > 90 ||
    message.location.lat < -90 ||
    message.location.lon > 180 ||
    message.location.lon < -180
  ) {
  ...
  console.log(
    `[WS] Received message ${message.content.text} from ${message.author}`
  );
  ...
  for (const recipient of recipients) {
    console.log(`found recipient ${recipient.uid} in range`);
